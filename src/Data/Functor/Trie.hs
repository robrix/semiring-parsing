...
instance (Monoid a, Semiring a, Functor i, Monoid (i (Trie i a))) => Semiring (Trie i a) where
  (h :< t) >< q = h ><< q <> (zero :< fmap (>< q) t)

instance (Unital a, Functor i, Monoid (i (Trie i a))) => Unital (Trie i a) where
  one = one :< zero

instance (Star a, Functor i, Monoid (i (Trie i a))) => Star (Trie i a) where
